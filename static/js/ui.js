$(async function () {
  ...
  if ($('#secure-form-btn')[0]) {
    $('#secure-form').on('submit', function () {
      $('#secure-loading')
        .children()
        .remove()
      $('#secure-loading').append(
        $(`<i class="fas fa-compass fa-3x fa-spin"></i>`)
      )
    })
  }
  ...
  $searchTrailForm.on('submit', async function (event) {
    ...
    $('#loading').append(
      $(`<i class="far fa-compass fa-4x fa-spin mt-3 light-icon"></i>`)
    )
    ...
    const results = await SearchTrailList.getTrails(
      $searchPlace.val(),
      $radius.val()
    )
    ...
    const search = await SearchTrailList.storeTrailSearch(
      results.data,
      $searchPlace.val(),
      $radius.val()
    )
  ...
  function applyOpenCloseSecureForm () {
    if ($('.open-secure')[0]) {
      $('.open-secure').on('click', function (e) {
        ...
        $('#secure-form').appendTo(
          $(`#info-modal-target-${e.currentTarget.id}`)
        )
        $('#secure-form').fadeIn()
        $('#secure-form-target').attr(
          'action',
          `/trails/${e.currentTarget.id}/secure`
        )
      })
